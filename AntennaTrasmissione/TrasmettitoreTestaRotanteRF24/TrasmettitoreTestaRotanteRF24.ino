...
void setup() {
  ...
  if(Serial.available()){
    do {
      Serial.readBytes(messaggioAvvio, 1);
      if(messaggioAvvio[0]=='5'){
        inFunzione=true;
      }
    }while (!inFunzione);
...
void messaggioWirless(uint8_t valore, int pos){
  if(pos==0){
   ...
   //Serial.println("2");
    ...
    if(statoMovimento==true && valore==0 || valore=='0'){
      //Serial.println("cambiato a false");
      messaggioWirlessVettore[5]='S';
      statoMovimento=false;
    }else if(statoMovimento==false && valore==1 || valore=='1'){
